...
def evaluate(name, ranker, kb, batch_size, predict_time=0, predict_time_pair=0, predict_rel=0, verbose=0,
    ...
    for i in range(0, int(facts.shape[0]), batch_size):
        ...
        for i in range(num_facts):
            ...
            # print("head:{}, rel:{}, tail:{}".format(valid_list[-1][0],valid_list[-1][1],valid_list[-1][2]))
            # print("Head rank:{}, Tail rank:{}".format(ranks_head[-1], ranks_tail[-1]))
            # print("top5_tail:{}, top5_head:{}, top3_rel:{}".format(top5_tail[-1],top5_head[-1], top3_rel[-1]))
            # xx=input()
